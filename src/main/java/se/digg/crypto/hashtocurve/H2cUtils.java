...
public class H2cUtils {
  ...
  public static boolean isSquare(final BigInteger val, final BigInteger order) {
    final BigInteger modPow = val.modPow(order.subtract(BigInteger.ONE).divide(BigInteger.TWO), order);
   ...
   *
   * @param parameterSpec the ECParameterSpec specifying the curve parameters
   * @param targetSecurityLevelBits the target security level in bits
   * @return the size of the L parameter in bytes
   */
  private int getCurveLsize(final ECParameterSpec parameterSpec, final int targetSecurityLevelBits) {
    return (int) Math.ceil(
        ((double) parameterSpec.getCurve().getOrder().subtract(BigInteger.ONE).bitLength() + targetSecurityLevelBits)
            / 8
    );
  }
